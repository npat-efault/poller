...
// available instead of waiting for more. Read compatible with the
// Read method of the io.Reader interface. In addition Read honors the
...
// returns the number of bytes written (0 <= nn <= len(p)) and any
// error encountered that caused the write to stop early.  Write
// returns a non-nil error if it returns nn < len(p). Write is
// compatible with the Write method of the io.Writer interface. In
// addition Write honors the timeout set by (*FD).SetDeadline and
...
// NOT atomic. If multiple goroutines write to the same FD their
// Writes may interleave. If you wish to protect agains this, you can
// embed FD and use a mutex to guard Write method calls:
//
//     struct myFD {
//         poller.FD
//         mu sync.Mutex
...
// Close closes the file descriptor. It is ok to call Close
// concurently with other operations (Read, Write, SetDeadline, etc)
// on the same file-descriptor. In this case ongoing operations will
// return with ErrClosed. Any subsequent operations (after Close) on
// the file-descriptor will also fail with ErrClosed.
...
// Lock locks the file-descriptor. It must be called before perfoming
